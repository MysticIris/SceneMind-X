"""Typed shared-fact structures for p3_shared_fact_v1_4_rc1."""
...
MediumType = Literal[
    ...
    "illustration_or_comic", "packaging", "mixed", "uncertain",
]
...
class DepictedContent(TypedDict):
    ...
    summary: str | None
    real_world_event_status: Literal["directly_observed", "depicted_only", "mixed", "uncertain"]
    evidence_refs: list[str]
    uncertainty_reason: str | None
...
class OcrCandidate(TypedDict):
    """Provide ocr candidate behavior."""
    text: str | None
    language: str | None
    ...
    source_type: Literal["model_visual", "ocr_tool", "human_gold"]
    region_ref: str | None
    confidence: float
    uncertainty_reason: str | None
...
class TextAssessment(TypedDict):
    ...
    text_types: list[Literal["printed", "handwritten", "artistic", "digital_interface", "mixed", "unknown"]]
    legibility: Literal["none", "low", "partial", "high", "mixed"]
...
class EvidenceItem(TypedDict):
    ...
    evidence_id: str
    source_type: Literal["visual_global", "visual_region", "model_text_assessment", "ocr_tool", "grounding_tool", "sam_tool", "human_gold"]
    content: str
    region_ref: str | None
    tool_ref: str | None
    confidence: float
...
class SelfCheck(TypedDict):
    """Provide self check behavior."""
    medium_content_separated: bool
    ocr_false_positive_guard_passed: bool
    unsupported_inference_absent: bool
    json_complete: bool
    issues: list[str]


class Rc1SharedFacts(TypedDict):
    """Provide rc1 shared facts behavior."""
